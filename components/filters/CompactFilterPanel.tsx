...
export function CompactFilterPanel() {
  ...
  // Clear selected segments when business type changes and segment type is "By End-Use*Product Type"
  useEffect(() => {
    if (selectedSegmentType === 'By End-Use*Product Type' && selectedSegments.length > 0) {
      // Clear segments when business type changes for this segment type
      setSelectedSegments([])
      setCurrentSegmentSelection('')
    }
  }, [filters.businessType, selectedSegmentType])
  ...
  if (selectedSegmentType === 'By End-Use*Product Type' && (filters.businessType === 'B2B' || filters.businessType === 'B2C')) {
    ...
    if (filters.businessType === 'B2B' && segmentDimension?.b2b_items) {
      ...
      const allHierarchyItems = new Set<string>()
      ...
      if (hierarchy[businessTypeRoot]) {
        // Add root children
        hierarchy[businessTypeRoot].forEach(item => allHierarchyItems.add(item))
        
        // Recursively add all descendants
        const addDescendants = (parent: string) => {
          if (hierarchy[parent]) {
            hierarchy[parent].forEach(child => {
              allHierarchyItems.add(child)
      ...
      Object.keys(hierarchy).forEach(key => {
        if (key !== businessTypeRoot && key !== 'B2B' && key !== 'B2C') {
          allHierarchyItems.add(key)
        }
      })
      
      availableSegments = Array.from(allHierarchyItems)
    }
  } else {
